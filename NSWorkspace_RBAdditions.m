...
static NSString* CheckParents(io_object_t thing,NSString* part,NSMutableDictionary* dict) {
    ...
    if ((kernResult==KERN_SUCCESS)&&parentsIterator) {
		...
		while ((nextParent = IOIteratorNext(parentsIterator))) {
            ...
            // NOTE: Conversion to ARC: Is it appropriate for ownership of reference
            // to be transferred to ARC at this point or is bridge okay?
            CFMutableDictionaryRef propRef = (__bridge CFMutableDictionaryRef)props;
			kernResult = IORegistryEntryCreateCFProperties(nextParent,&propRef,kCFAllocatorDefault,0);
            props = (__bridge NSMutableDictionary*)propRef;
			...
			if (IOObjectConformsTo(nextParent,"IOApplePartitionScheme")) {
				...
				if ((data = [[props objectForKey:@"Protocol Characteristics"] objectForKey:@"Virtual Interface Location Path"])) {
					image = [[NSString alloc] initWithBytes:[data bytes] length:[data length] encoding:NSUTF8StringEncoding];
				}
			...
			NSString* cls = (NSString*)CFBridgingRelease(IOObjectCopyClass(nextParent));
...
static NSString* FormatAFPURL(AFPXVolMountInfoPtr mountInfo,NSString** devdesc) {
	...
	if (devdesc) {
		*devdesc = [[NSString alloc] initWithBytes:&work[1] length:work[0] encoding:NSUTF8StringEncoding];
	}
	work = ((UInt8*)mountInfo)+mountInfo->volNameOffset;
	NSString* volname = [[NSString alloc] initWithBytes:&work[1] length:work[0] encoding:NSUTF8StringEncoding];
	...
	while ((i-->0)) {
		switch (afpta->fType) {
			case kAFPTagTypeIP:
				if (!ip) {
					ip = [[NSString alloc] initWithBytes:&afpta->fData[0] length:afpta->fLength-2 encoding:NSUTF8StringEncoding];
				}
				break;
			case kAFPTagTypeIPPort:
				...
				break;
			case kAFPTagTypeDNS:
				dns = [[NSString alloc] initWithBytes:&afpta->fData[0] length:afpta->fLength-2 encoding:NSUTF8StringEncoding];
				break;
			case 0x07:
